...
(function () {
	...
	function init() {
		...
		eTemperature.innerHTML =
			Math.round(current.data.instant.details.air_temperature) + "&deg;";
		...
		if (
			current.data.next_1_hours.summary.symbol_code.indexOf("sunny") != -1 ||
			...
			current.data.next_1_hours.summary.symbol_code.indexOf("fog") != -1
		) {
			eBody.className = "bg-fog";
		} else if (
			current.data.next_1_hours.summary.symbol_code.indexOf("rain") != -1
		) {
			...
			current.data.next_1_hours.summary.symbol_code.indexOf("snow") != -1
		) {
			eBody.className = "bg-snow";
		} else if (
			current.data.next_1_hours.summary.symbol_code.indexOf("storm") != -1
		) {
			eBody.className = "bg-storm";
		} else if (
			current.data.next_1_hours.summary.symbol_code.indexOf("cloudy") != -1
		) {
			eBody.className = "bg-cloudy";
		} else if (
			current.data.next_1_hours.summary.symbol_code.indexOf("overcast") != -1
		) {
		...
		Highcharts.chart("container", {
			...
			xAxis: {
				...
				labels: {
					...
					formatter: function () {
						return (
							'<div class="w-icon w-icon-' +
							this.value[1] +
							'"></div><span class="w-time">' +
							this.value[0] +
							"</span>"
						);
					},
...
// 			if (max_temp < temp) max_temp = temp + adjust_temp + 10;
// 		}
